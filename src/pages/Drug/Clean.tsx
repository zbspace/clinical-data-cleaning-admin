...
const DrugClean: React.FC = () => {
  ...
  const columns = [
    ...
    { colKey: 'originalName', title: '药品名（源数据）', width: 180, cell: ({ row }: any) => row.drugNickName || row.drugComment || row.originalName },
    {
      colKey: 'relatedNo',
      title: '相关受理号/登记号',
      width: 150,
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => {
        ...
        return <Tag theme={theme as any} variant="light">{statusText}</Tag>;
      },
    },
    { colKey: 'cleanedName', title: '药品名（清洗后）', width: 180, cell: ({ row }: any) => row.drugStandardName || row.cleanedName },
    { colKey: 'genericNameCn', title: '通用名（中文）', width: 150, cell: ({ row }: any) => row.drugNormalNameCn || row.genericNameCn },
    { colKey: 'genericNameEn', title: '通用名（英文）', width: 150, cell: ({ row }: any) => row.drugNormalNameEn || row.genericNameEn },
    { colKey: 'rdCode', title: '研发代号', width: 120, cell: ({ row }: any) => row.drugCode || row.rdCode },
    { colKey: 'otherNames', title: '其他名（例如结构描述）', width: 180, cell: ({ row }: any) => row.otherComment || row.otherNames },
  ...
  return (
    <Card bordered={false} style={{ padding: '10px' }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ margin: '0 0 16px 0', fontSize: '18px', fontWeight: 600, color: 'var(--td-text-color-primary)' }}>药品名清洗</h2>
        <div style={{ 
          background: '#f8fafc', 
          padding: '16px', 
          borderRadius: '12px',
          ...
          <Form form={filterForm} layout="inline" labelWidth={140} style={{ display: 'flex', gap: '16px 0', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', marginLeft: 'auto' }}>
              <Button theme="default" onClick={() => { filterForm.reset(); fetchData(); }} style={{ background: '#fff', marginRight: 8 }}>重置条件</Button>
              <Button theme="primary" onClick={handleSearch}>立即查询</Button>
            </div>
          </Form>
        </div>
      </div>
      ...
      >
        <div style={{ backgroundColor: '#f3f4f6', padding: 16, marginBottom: 16, borderRadius: 4 }}>
          <p style={{ margin: '0 0 8px 0' }}><strong>药品名（源数据）：</strong> {currentEditRecord?.originalName}</p>
          <p style={{ margin: 0 }}><strong>药品类型：</strong> {currentEditRecord?.drugType || '化学药物'}</p>
        </div>
        <Form form={editForm} labelWidth={180} onSubmit={(e) => {
          console.log(e);
